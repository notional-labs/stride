...
func (k Keeper) GetHostZoneUnbondingMsgs(ctx sdk.Context, hostZone types.HostZone) (msgs []sdk.Msg, totalAmountToUnbond sdk.Int, marshalledCallbackArgs []byte, epochUnbondingRecordIds []uint64, err error) {
	...
	if err != nil {
		...
		return nil, sdk.ZeroInt(), nil, nil, fmt.Errorf("%s: %s", errMsg, types.ErrNoValidatorAmts.Error())
	}
	...
	if overflowAmount.GT(sdk.ZeroInt()) {
		...
		k.Logger(ctx).Error(errMsg)
		return nil, sdk.ZeroInt(), nil, nil, fmt.Errorf("%s: %s", errMsg, "not found")
	}
	...
	if delegationAccount == nil || delegationAccount.Address == "" {
		...
		k.Logger(ctx).Error(errMsg)
		return nil, sdk.ZeroInt(), nil, nil, fmt.Errorf("%s: %s", errMsg, types.ErrHostZoneICAAccountNotFound.Error())
	}
	...
	if err != nil {
		...
		return nil, sdk.ZeroInt(), nil, nil, fmt.Errorf("%s : %s", err.Error(), "not found")
	}
