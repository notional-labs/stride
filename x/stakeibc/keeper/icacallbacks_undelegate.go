...
func (k Keeper) UpdateHostZoneUnbondings(
	...
	for _, epochNumber := range undelegateCallback.EpochUnbondingRecordIds {
		...
		if !found {
			...
			k.Logger(ctx).Error(errMsg)
			return sdk.ZeroInt(), fmt.Errorf(`%s: %s`, errMsg, types.ErrKeyNotFound.Error())
		}
		...
		if !success {
			...
			return sdk.ZeroInt(), fmt.Errorf("couldn't set host zone epoch unbonding record. err: %s: %s", err.Error(), types.ErrEpochNotFound.Error())
		}
