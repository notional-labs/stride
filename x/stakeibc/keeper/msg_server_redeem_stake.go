...
func (k msgServer) RedeemStake(goCtx context.Context, msg *types.MsgRedeemStake) (*types.MsgRedeemStakeResponse, error) {
	...
	if !nativeAmount.IsPositive() {
		return nil, fmt.Errorf("amount must be greater than 0. found: %v: invalid coins", msg.Amount)
	}
	...
	if balance.Amount.LT(msg.Amount) {
		return nil, fmt.Errorf("balance is lower than redemption amount. redemption amount: %v, balance %v: : invalid coins", msg.Amount, balance.Amount)
	}
