...
for (( i=1; i <= $NUM_NODES; i++ )); do
    ...
    if [[ $CHAIN != "STRIDE" ]]; then
        cp $DOCKERNET_HOME/state/${STRIDE_NODE_PREFIX}${i}/config/priv_validator_key.json $DOCKERNET_HOME/state/$node_name/config/priv_validator_key.json
        cp $DOCKERNET_HOME/state/${STRIDE_NODE_PREFIX}${i}/config/node_key.json $DOCKERNET_HOME/state/$node_name/config/node_key.json
    fi
